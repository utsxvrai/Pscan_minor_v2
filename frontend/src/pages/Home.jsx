...
import { Upload, LogIn, UserPlus, X, Dumbbell } from 'lucide-react';
import { predictPneumonia } from '../api';
import ChatbotCard from '../components/ChatbotCard';
import HospitalFinder from '../components/HospitalFinder';
import RecoveryExercises from '../components/RecoveryExercises';
import RecoveryVideos from '../components/RecoveryVideos';
...
const HomePage = () => {
  ...
  <div
    ...
    onDrop={(e) => {
      ...
      handleFileDrop(e); // Handle file drop if logged in
    }}
    onClick={() => {
      if (!isLoggedIn) {
        setIsModalOpen(true); // Show modal if not logged in
  ...
  >
    ...
    <input
      ...
      onChange={(e) => {
        ...
        handleFileChange(e); // Handle file upload if logged in
      }}
    />
  </div>
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-gray-800 text-white rounded-lg p-6 w-96">
        ...
        <div className="space-y-6">
          ...
          {/* Exercise Card */}
         <RecoveryExercises onScrollToVideos={scrollToVideos} />

          {/* News Card */}
          <div className="bg-amber-100/10 rounded-3xl p-6 backdrop-blur-sm">
            <h3 className="font-semibold mb-4">Go to your profile</h3>
            <button
              onClick={() => navigate('/profile')} // Place onClick here
              className="flex items-center gap-2 bg-white/10 rounded-lg px-4 py-2 hover:bg-white/20 transition-colors"
              >
      ...
      <div ref={videoSectionRef} className="mt-16">
        <RecoveryVideos />
      </div>
      <ChatbotCard />
    </div>
